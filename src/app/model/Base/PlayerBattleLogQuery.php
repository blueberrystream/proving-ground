 ...
 * @method     ChildPlayerBattleLogQuery orderByWin($order = Criteria::ASC) Order by the win column
 ...
 * @method     ChildPlayerBattleLogQuery groupByWin() Group by the win column
 ...
 * @method     ChildPlayerBattleLog findOneByWin(boolean $win) Return the first ChildPlayerBattleLog filtered by the win column
 ...
 * @method     ChildPlayerBattleLog requireOneByWin(boolean $win) Return the first ChildPlayerBattleLog filtered by the win column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildPlayerBattleLog[]|ObjectCollection findByWin(boolean $win) Return ChildPlayerBattleLog objects filtered by the win column
...
abstract class PlayerBattleLogQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, player_id, enemy_player_id, win, created_at, updated_at FROM player_battle_log WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByWin(true); // WHERE win = true
     * $query->filterByWin('yes'); // WHERE win = true
     * </code>
     *
     * @param     boolean|string $win The value to use as filter.
    ...
    {
        if (is_string($win)) {
            $win = in_array(strtolower($win), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }

        return $this->addUsingAlias(PlayerBattleLogTableMap::COL_WIN, $win, $comparison);
    }
